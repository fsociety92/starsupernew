...
class MyAdsActivity : AppCompatActivity() {
                ...
                mainHandler.post(Runnable {
                    ...
                    if (ads.length() == 0)
                        ...
                        for (i in 0 until ads.length()) {
                            ...
                            val deleteButton = cardView.findViewById<Button>(R.id.btn_delete_ad)
                            deleteButton.setOnClickListener {
                                val adId = ad.getString("uuid") // Получение идентификатора рекламы
                                deleteAd(cardView, adId) // Вызов метода для удаления рекламы по идентификатору
                            }
                        }
                })
            }
    ...
    private fun deleteAd(cardView: CardView, adId: String) {
        val request = Request.Builder()
                .addHeader("Authorization", "Bearer $accessToken")
                .url("${rootUrl}/ads/$adId")
                .delete()
                .build()
        ...
        okHttpClient.newCall(request).enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                if (response.isSuccessful) {
                    // Удаление рекламы успешно выполнено
                    runOnUiThread {
                        // Удаление CardView из LinearLayout, чтобы она больше не отображалась
                        findViewById<LinearLayout>(R.id.ll_ads).removeView(cardView)
                    }
                } else {
                    // Обработка ошибки удаления рекламы
                    println("Error deleting ad: ${response.code}")
                }
            }
        })
    }
